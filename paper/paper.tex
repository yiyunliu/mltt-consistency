...
$[[A]]$. In the Coq mechanization, we represent sets of terms as predicates
over terms asserting whether the term is in the set, so $[[S]]$ has type
\texttt{tm -> Prop}, and $[[I]]$ has type \texttt{forall j, j < i -> tm -> Prop},
representing the set of types in universes strictly smaller than $[[i]]$.
Because the overall inductive definition represents a functional relation
between a type at a universe level and its interpretation as a set of terms,
it too lives in \texttt{Prop}.
...
of code remains small at below 1200 lines, and functional extensionality is
...
universes with subtyping, and $\eta$ law for functions. All these
...
\begin{figure}
  \drules[T]{$[[G |- a : A]]$}{Typing for Naturals}{Nat, Zero, Suc, Ind}
  \drules[I]{$[[Interp I i A S]]$}{Logical Relation for Naturals}{Nat}
...
\Rref{I-Nat} gives us the semantic
interpretation of natural number; a term $[[a]]$ semantically inhabits
$[[Nat]]$ if $[[a]]$ reduces to some term $[[b]]$ such that $[[NatOk
b]]$ holds.
The $\vdash_\mathbb{N}$ judgment inductively characterizes the
...
dependent type theories. % Intuitively, proof-relevant logical relations
% can be viewed as the set theoretic augmented with information
% about syntactic terms so the fundamental theorem can give us more
Compared
