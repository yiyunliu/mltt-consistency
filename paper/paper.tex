...
\usepackage{xspace,stmaryrd,mathtools,bbding}
...
Consider first the following simplified \rref{I-PiAlt}, which follows from \rref{I-Pi}.
...
a well-behaved dependent function type. The preconditions of
the rule says that the function type $[[Pi x : A . B]]$ has an interpretation
if its input type $[[A]]$ can be interpreted as some set $[[S]]$, and for all
terms $[[a in S]]$, the type $[[B {a / x}]]$, obtained by substituting $[[a]]$
into the output type $[[B]]$, has some semantic interpretation $[[S0]]$. In its
conclusion, the  interpretation of $[[Pi x : A . B]]$ is the set of terms
$[[b]]$ such that for all $[[a in S]]$, where $[[S]]$ is an interpretation of
$[[A]]$, the application form $[[b a]]$ belongs to all possible interpretations
of $[[B {a  / x}]]$. The second precondition ensures at least one interpretation
exists for each $[[B {a / x}]]$ where $[[a in S]]$.

\jc{I think at this point it's unclear why \rref{I-PiAlt} is shaped the way it is.
I found myself asking: If in the second precondition we've established the existence of $[[S0]]$,
why do we need to quantify over all interpretations $[[S0]]$?
In fact, instead of the second precondition,
why can't split it into two and say that there exists some family of sets
$S_0 : \forall a, a \in S \to [[PowerSet STm]]$
such that $\forall a, (m : a \in S) \to [[Interp I i B { a / x } (# S0 a m #)]]$,
then conclude that the intepretation of the function type yields a set over $[[b]]$
such that $\forall a, (m : a \in S) \to [[b in S0 a m]]$?
As far as I can tell, the interpretation would be well formed,
but proving the fundamental theorem would then require something like indefinite description,
since the induction hypothesis would yield $\forall a, a \in S \to \exists S_0, \dots$,
the mere existence of such a family of sets,
while the constructor requires the actual family of sets itself.
This is subtle but maybe warrants explaining.}
