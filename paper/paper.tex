...
forms. We discuss how the inclusion of type annotations can affect our
...
we define in Definition~\ref{fig:logrelrec} the final version of our interpretation judgment recursively
...
By unfolding Definition~\ref{fig:logrelrec}, we
...
\end{lemma}
\begin{proof}
  By the definition of $[[InterpR i A S]]$ 
  (Definition~\ref{fig:logrelrec}), we unfold $[[InterpR i A S]]$ by one
...
closed terms from Section~\ref{sec:extension}.

The development with the $\beta\eta$-normalization proof from Section~\ref{sec:betaeta} comprises 1573
lines of non-blank, non-comment lines of code. We choose not to
include it in the chart, since the slight difference in lemma
dependencies makes the comparison in each category less
informative. However, we note that compared to the
$\beta$-normalization extension, the $\beta\eta$ extension has the
same line count in the definition of the logical relation and the
semantic soundness proof. The difference only appears in files related
to untyped reduction and normal forms.
...
In the Coq encoding of $[[InterpR i A S]]$ (Definition~\ref{fig:logrelrec}), we define $[[I]] \in [[SNat -> PowerSet STm]]$ as follows.
\begin{equation*}
  \begin{split}
    [[I j]] &=
     \begin{cases}
...
Compared to the systems we describe in this section, the most notable
features we are missing include cumulativity and impredicativity. Our
semantic model already satisfies the cumulativity property
(Lemma~\ref{lemma:logrelcumulativity}), though we need to extend our
convertibility relation into a subtyping relation in the specification
of our syntactic typing rules. Impredicativity, on the other hand, is
known to be difficult to model when the impredicative sort is at the
bottom of a predicative universe hierarchy; in this scenario, the
erasure technique from \citet{geuvers1994short} is no longer
applicable\citep{abel2013normalization}. Whether we can find a
similarly short and simple treatment for impredicativity remains an
open question, which we hope to explore in the future.
