...
equipped with dependent elimination forms. % The mechanization is
% extremely short and involves less than 1000 lines of manually written
% Coq code.
...
to readers with general mathematical background. However, notation
aside, the typeset representation is purposefully made to be closer to
our mechanization so less massaging of the definitions is required to
encode our proofs in a proof assistant.  The close correspondence
between a typeset proof and a mechanized proof comes at the potential
risk that the artifacts specific to theorem provers are reflected upon
the presented proof. However, for proofs by logical relation for
dependent types, we find our approach advantageous for a few
reasons.
...
agnostic or merely ambiguous about the underlying metatheory
...
In Section~\ref{sec:logreldep}, we give the definition of our logical
relation as an inductively defined relation. We first show that the
logical relation respects evaluation (referred to as the irrelevance
property in \citet{martin-lof-a-la-coq}), then proceed to
prove that the logical relation corresponds to a partial function in
the sense that the each type has a unique interpretation if there
exists one.
...
In Section~\ref{sec:extension}, \scw{what about sections 2-4?}
 we extend our logical
...
the informal presentation in our paper.  \scw{Give an overview of the figure}
...
syntax, summarized in Figure~\ref{fig:auxdef}.
  ...
  \begin{equation*}
    ...
    \begin{split}
      \begin{array}{lll}
        ...
        [[(a b) < xi >]] & := & [[a < xi > (# b < xi > #)]] \\
        \ldots \\ \\
        \mathit{UpTm \scw{shifting? lifting?}} \\
        [[( up rho ) 0]] & := & [[var 0]] \\
...
type where $[[i]]$ ranges over the set of natural numbers.
 Finally,
\lang\scw{Don't begin a sentence with a symbol} also includes an intensional identity type $[[a ~ b : A]]$ whose
...
exactly the same as untyped $\beta$-equivalence.\scw{cite barendregt?}
...
i ]] := [[G i]] $.  \scw{explain the renaming in the conclusion of
this lemma}
The precondition $[[i < | G | ]]$, where $[[ | G |
...
\scw{Because the type of $I$ depends on $i$, I'd prefer a syntax that
keeps them in order. }
...
\scw{Spell this out a bit more about what goes wrong. Where does the
proof break? Also spell out how this enforces predicativity.}
...
\label{fig:logrelrec}
\scw{I didn't understand this figure without reading the text below.}
\end{figure}
...
principles.\scw{Put footnotes on lemma name, or on (1), (2), etc}
...
\end{lemma}
\begin{proof}
  \scw{Why is this proof interesting and worth typesetting? Should
    it be in an appendix so that it doesn't interrupt the flow of
    reading?}
  ...
  \item[\Rref{I-Red}:] We are given that $[[Interp I i Pi A B S0]]$.
    ...
    \item $[[forall a, (# a in S implies Interp I i B { a } F a #)]]$
    \end{itemize}
    These properties are exactly what's\scw{avoid contractions} required in the conclusion.
  \end{description}
\end{proof}
...
interpretation.
\scw{I'm not used to this lemma being called irrelevance. Intuitively,
it says that the definition of the logical relation respects
...
\scw{How is this related to the comment before 3.2 (i.e. the property
derived from I-Red?}
...
\scw{Put this introduction before the figure in the section.}
...
\end{center}
\scw{What happened to the domain type in the $\lambda$ expression? This is
not your syntax...}
