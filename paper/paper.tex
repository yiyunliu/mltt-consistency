...
the universe level, a natural number $i$.%  and $[[x]]$
...
%        [[idtm x]] & := & [[x]] \\ \\
%        \multicolumn{3}{l}{\mbox{\emph{Extension operation: }} ([[rho .: x -> a]]) } \\
%        ([[rho .: x -> a]])([[x]]) & := & [[a]] \\
...
the relation $[[F]]$, analogous to the admissible rule \rref{I-PiAlt}.
...
% $[[S]]$ and the universe level $[[i]]$ in the definition of $[[rho |= G]]$, we only need show that there exists some $[[i]]$ and $[[S]]$
% such that $[[InterpR i A { rho } S]]$ and $[[rho x in S]]$ since the logical relation is a partial function (Lemma~\ref{lemma:logreldeter}). This idea is encapsulated in the second of the two structural
% law about $[[rho |= G]]$.
...
not normalizing.
% \scw{I don't see this precondition in the figure!}\yl{resolved}
...
sort. In particular, the relation $[[R]]$ in \rref{I-Pi} requires the use of
impredicativity because $[[R]]$ must later be instantiated into something
...
impredicativity is its countable universe hierarchy. \scw{I'm not sure the
  last part of this sentence adds much: ``Although, lemmas such as the
  admissibility of \rref{I-PiAlt} (Lemma~\ref{lemma:piintroalt}) not only
  simply our mechanization, but also give better intuition on what our logical
  relation means.'' Yes, we need impredicativity to define these lemmas, but
  without impredicativity our logical relation means something else.}
...
factorization property until we need to justify the use leftmost-outermost
reduction as a deterministic reduction strategy for normalization.
