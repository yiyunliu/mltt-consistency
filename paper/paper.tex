...
\rref{T-Abs}, where the induction hypothesis is not helpful since the body
...
law about $[[rho |= G]]$.
\begin{lemma}[Well-formed $[[rho]]$ empty\footnote{\dotv{soundness.v}{$\gamma$\_ok\_nil}}]
...
Analogous to $[[|- G]]$, the syntactic well-formedness of the typing
context, $[[|= G]]$ defines when a context is semantically
well-formed. In the
...
returns the empty list. Intuitively, the definition of $[[|= G]]$
simply says all types in the context must correspond to some set
according to the logical relation. The truncation to the context
$[[G]]$ is due to the different scoping of the types in the
context. An alternative way of defining $[[|= G]]$ would look more
similar to $[[|- G]]$, though we find it easier to define $[[|= G]]$
through for all quantification of $[[i]]$ and recover the structural
rules as lemmas.
...
\begin{proof}
  ...
  the assupmtion that $[[a in S]]$. Done.
\end{proof}
...
\begin{proof}
  ...
  the term (evident from \rref{P-AbsEta}).
\end{proof}
...
\begin{figure}[h]
  \begin{tabular}{ c |  c  | c  }
    ...
    Neutral and normal forms & N/A & 269 \\
    Logical relation & 331 & 426 \\
    Semantic typing and soundness & 169 & 199 \\
    \hline
    Total & 963 & 1357 \\
  \end{tabular}
...
hierarchy, though lemmas such as the admissibility of \rref{I-PiAlt}
...
graph of \mintinline{coq}{tstar a}, the Takahashi translation. Options \texttt{inv:Par} and
...
\yl{Mention that logrel itself doesn't require impredicativty}

\subsection{Extraction of conversion algorithm}
\yl{technically future work. Not sure whereelse to put it}
...
technique. We've already demonstrated how to address type-level
dependency when defining a logical relation through a simple
example. Claiming that our proof structure is better seems quite
ambitious, but I think there's a middle point where we claim that
adding moderate features like typed reduction doesn't instantly make
our code size expand all the way from 1000 to 20,000 without saying
the other developments are just verbose for no good reason}
The short consistency proof we
present achieves the goal of demonstrating the technique of proof by
logical relation for dependently typed languages. However, what
remains unanswered is what makes our development significantly shorter
compared to others. Are we proving simpler results, or is our proof
...
First, the metatheoretic property we prove is indeed simpler.
Unlike developments that mechanize the correctness of a
type-directed conversion algorithm, we only show the existence of
...
of scaffolding related to the specification of the algorithm and the
extra proof obligation that the algorithm is sound and complete with
...
predicates rather than relations. The need for a relational model is
...
and terms in \rref{I-Par, I-Bool} using the non-deterministic parallel reduction relation,
which relates to full reduction, whereas \citet{decagda,nbeincoq} all
refer to the deterministic weak head reduction. The use of a
non-deterministic reduction strategy comes at the cost that we need
confluence to prove the functionality of our logical relation, but
immediately gives us a semantic justification of the conversion rule
(Lemma~\ref{lemma:logrelcoherence}).
...
other mechanized proofs. However, our early dependence on confluence
before the fundamental theorem is established can be alarming.
In a system with type-directed reduction,
confluence is not immediately available since it depends on
...
to syntactically prove $\Pi$-injectivity for arbitrary Pure Tyep
Systems. \citet{weirich:systemd} add $\Pi$-injectivity to their
equational theory, thus allowing subject reduction to be proven
independently from confluence. Therefore, we believe that even in a
