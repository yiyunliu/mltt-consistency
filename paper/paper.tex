...
decidability of conversion algorithms~\citep{harper2005equivalence,Abel12,abel2013normalization}. \scw{% add citations?
...
\scw{I'm not sure that this is the best place for this discussion.}
...
reduction~\citep{takahashi-parallel-reduction, barendregt:lambda-calculi-with-types} % to normal form~\citet{barendregt}
.
...
category. The type $[[Set i]]$ represent universe types.%  and $[[x]]$
...
% variables.
% We use de Bruijn indices for our Coq development, but use nominal
% representation in the paper because it is more amenable to automation
...
example of a substitution is the function $\ottkw{id}_{tm}$,\scw{Can we just 
call this $\ottkw{id}$ now?} the identity
substitution that maps each index to the corresponding term variable.  % The
% $...$ substitution is more general and adds $[[n]]$ to the index
% before injecting it as a term.
...
\scw{This substitution operation is pretty strange when it comes to the 
BVC. What variables are we allowed to use when we get to a bound variable $x$ in this function? I'd prefer if we instead had a side condition that it must be 
the case that $\rho(x) = x$ already, then we use BVC to pick such an $x$. But then we need to know that such an $x$ exists, and that isn't necessarily true.}
\scw{Maybe we should handwave here, and note that we aren't covering all of the 
details of the treatment of variable binding in the text. If readers want to understand that part, they should look at the Coq code, which uses de Bruijn indices anyways. Our goal is to convey the understanding of the Coq proof. }
...
proofs, as we discuss in Section~\ref{sec:discuss}. % \scw{Forward reference to further
  % discussion, comparing with whnf reduction?} \yl{resolved?}
...
the % run-time \yl{commented out run-time per Jonathan's comment}
...
such that $[[InterpR i A { rho } S]]$ and $[[rho x
...
\drules[SCtx]{$[[|= G]]$}{Semantic context well-formedness}{Empty, Cons}
% \[
%    \forall x : A in G,\text{ there exists some } i \text{ such that } drop Suc n G |= G n : Set i \]
\end{definition}
...
%      & \header{Large Elim} & Main results \\
    ...
    % % \hline
    % % Large Elimination & {\boxedsymbols ✓} & {\boxedsymbols ✓} & {\boxedsymbols ✓} \\
    % \hline
  ...
  \begin{tabular}{ll}
    \\
  \underline{Univ}erses: & Countable ($\mathbb{N}$), Zero (0), One (1), Impredicative ($\mathbb{I}$) \\
  \underline{Ind}uctives: & Identity types (Id), Natural numbers (Nat), W-types \\
  \underline{Conv}ersion:& Untyped (U), Typed (T),  Extensional (E) \\
  \underline{Large E}liminations:& Included ({\boxedsymbols ✓}), not included ({\boxedsymbols ✗}) \\
  \\
